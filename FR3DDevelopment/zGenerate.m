...
for r = 9:length(x(:,1)),
  ...
  if C1 > C2,
    Class = -Class;
    Paircode = 4*(C1-1) + C2;                    % AA is 1, CA is 2, etc.
    a1 = x{r,8};                                  % reverse order of atoms
    a2 = x{r,7};
    a3 = x{r,5};
    a4 = x{r,4};
  else
    Paircode = 4*(C2-1) + C1;                    % AA is 1, CA is 2, etc.
    a1 = x{r,4};                                  % reverse order of atoms
    a2 = x{r,5};
    a3 = x{r,7};
    a4 = x{r,8};
  end
...
%  h
  ...
  if (strcmp(a1,'H2*')==1) | (strcmp(a2,'H2*')==1) | ...
     ...
     fprintf('Cannot check hydrogen bond for H2*\n');
%    [x{r,1} ' ' x{r,2} ' ' x{r,3} ' ' a1 a2 a3 a4]
  else
    ...
    fprintf(fid,'  Hydrogen(%1d).Distance = dist(',h);
    fprintf(fid,'N1.%s,',zGetAtomStorageLocation(a2,C1));
