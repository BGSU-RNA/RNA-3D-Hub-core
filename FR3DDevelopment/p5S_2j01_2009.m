...
% For Alignment 4 and higher, we need to put in an interaction between
% the hairpin at 40 and something, so it does not treat it as extensible

AlignmentList = 1:7;                           % which alignments to make
AlignmentList = 4:4;

MF = ['5S_2J01_JAR3D'];                                % model file
SeqFile    = '5S_Bacterial_Stombaugh_et_al_Sup_Mat_S1_2J01_first.fasta';
ModelStart = '1_B';
ModelEnd   = '119_B';
NumSeq     = 20;

if ~exist('File')
  File = zAddNTData('2J01');
end
...
% ----------------------------------------------- JAR3D 1 alignment
%       NIH BISTI Method 4 scoring, no clusters, 
...
if any(jj == AlignmentList),
  ...
  F = pRemoveTriples(F,0);

%  F = pModifyEdge(F,'24_A','27_A',0);       % remove nested triple
  ...
  fprintf('JAR3D 1 - basepair substitutions method 4, no clusters\n');
  pDisplayAlignment
...
if any(jj == AlignmentList),
  ...
  fprintf('JAR3D 2 - clusters, no adjustment due to LR interactions\n');
  pDisplayAlignment
end

% ----------------------------------------------- JAR3D 3 alignment
%        NIH BISTI Method 1 scoring with adjustment
%        of basepairing probabilities due to LR interactions and BPh

jj = 3;

if any(jj == AlignmentList),
  JNode = pMakeNodes(F,[Verbose 4 0 1],ModelStart,ModelEnd);
  T([1 jj+1],:) = pMakeNodesDiagnostics(F,JNode);
  JNode = pShiftNodeIndices(JNode,Shift);
  ...
  fprintf('JAR3D 3 - clusters, adjustment due to LR interactions\n');
  pDisplayAlignment
end
...
if any(jj == AlignmentList),
  ...
  fprintf('JAR3D 4 - clusters, adjustment due to LR interactions, extension of stems\n');
  pDisplayAlignment
end
