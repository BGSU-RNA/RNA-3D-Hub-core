% zDisplayHet(File,HetList,ViewParam) is a general-purpose nucleotide plotting program.  It can be called in several ways, for example,
% zDisplayHet('1s72',{'27','28','29'},ViewParam), and it will load the
% named datafile and plot the nucleotides by nucleotide number.  Or, if
% data files have already been loaded, one can use zDisplayHet(File(1),[32
% 34 35],ViewParam) to plot the nucleotides in File(1) having indices 32,
% 34, and 35.  Defaults for ViewParam are defined in zDisplayHet; see there
% for the fields of ViewParam.
% One can also use ranges of nucleotide numbers, as in
% zDisplayHet('rr0033_23S',{'2548:2555','2557','2559:2566'},VP);

function [void] = zDisplayHet(File,HetList,ViewParam)
...
if nargin == 1 || isempty(HetList),
  HetList = 1:length(File.Het);                  % display them all
end

% if HetList is a cell array of numbers, look up the indices

if strcmp(class(HetList),'char'),
  HetList = {HetList};
end

if strcmp(class(HetList),'cell'),
  Indices = zIndexLookup(File,HetList);
else
  Indices = HetList;
end
...
if VP.AtOrigin > 0,
  v = fix(VP.AtOrigin);
  R = File.Het(Indices(v)).Rot;             % Rotation matrix for first base
  S = File.Het(Indices(v)).Loc(1,:);        % Location of glycosidic atom
else
  R = VP.Rotation;
  S = VP.Shift;
end
...
for j=1:length(Indices),                 % Loop through all nucleotides
  ...
  zPlotOneHetRotated(File.Het(k),VP,R,S);
end

Title = strcat(File.Het(Indices(1)).Unit,File.Het(Indices(1)).Number);
for j=2:length(Indices),
  nt = File.Het(Indices(j));
...
if VP.WritePDB == 1,
  if VP.AtOrigin > 0,
    v = fix(VP.AtOrigin);
    G = eye(3);
    R = File.Het(Indices(v)).Rot * G(:,[2 3 1]);% Rotation matrix for first base
    S = File.Het(Indices(v)).Loc(1,:);        % Location of glycosidic atom
  else
    R = VP.Rotation;
    S = VP.Shift;
  ...
  for j=1:length(Indices),                 % Loop through all nucleotides
    k = Indices(j);                        % index of current nucleotide
    a = zWriteNucleotidePDB(fid,File.Het(k),a,0,R,S);
  end
...
if VP.Animate == 1,
  ...
  if isfield(ViewParam,'Rotation'),
    ...
    R = File.Het(Indices(1)).Rot * Spin([3 1 2], [3 1 2]);
  end
  ...
  maxheight = 0;
  for j = 1:length(Indices);
    L  = length(File.Het(Indices(j)).Loc(:,1));         % Number of base atoms
    V  = (File.Het(Indices(j)).Loc - ones(L,1)*S) * R;
