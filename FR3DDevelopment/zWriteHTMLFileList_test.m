...
RepAndBetter = 2;               % if > 1, only list representative and better structures
...
for r = 1:length(Reports(:,1)),
  ...
  case 1,                                             % list all files
    ...
    [tt,nn] = zEquivalents(t(i,:),n(i,:),6,0);        % files equivalent to these
    fid = fopen([destination filesep 'index.html'],'w');
    fprintf(fid,'<html>');
    fprintf(fid,'<head>\n');
    fprintf(fid,'<title>List of all %d PDB structures containing RNA nucleotides as of %s</title>\n',length(tt(:,1)),reportdate);
...
%          if nn(j,1) < currentres,                   % better resolution
  ...
  case 2,
    ...
    [tt,nn] = zEquivalents(t(i,:),n(i,:),6,1);      % get equivalents
  ...
  fprintf(fid,'<table class="sorttable" border=1>');

  fprintf(fid,'<thead><tr><th class="narrow">PDBID</th><th class="narrow sorttable_nosort">FR3D analysis</th><th class="narrow sorttable_nosort">Chain(s)</th><th class="narrow">NTs</th><th class="narrow">Pairs</th><th class="narrow">Res</th><th class="narrow">Date</th><th>Organism</th><th class="sorttable_nosort">Description provided by author</th><th class="sorttable_nosort">Author</th>');

  switch Format,
  case 1,
    fprintf(fid,'<th class="narrow sorttable_nosort">Represented by</th></tr>\n');
  case 2,
    fprintf(fid,'<th class="sorttable_nosort">Also represents</th></tr></thead>\n');
  end
  ...
  for r = 1:length(tt(:,1)),
    ...
    fprintf(fid,'<td><a href="%s/%s/index.html" title="Link to FR3D analysis. %0.4f pairs per nucleotide">%s</a></td>', webroot,upper(tt{r,1}),nn(r,8)/nn(r,7),tt{r,1}); % CLZ 2011-05-17
...
%  res = strrep(res,'SOLUTION NMR','NMR');
    ...
    case 2,
      ...
      fprintf(fid,'<td>');
      if length(g) > 1,
        [y,i] = sort(-n(g,8)./n(g,7));                       % sort by pairs per nucleotide
        g = g(i);
        for h = 1:length(g),
          if ~strcmpi(t{g(h),1},tt{r,1}),
            ...
            fprintf(fid,'<a href="%s/%s/index.html" title="%s, %d nucleotides, %d basepairs, %0.4f pairs per nucleotide">%s</a> ', webroot,upper(t{g(h),1}),res,n(g(h),2),n(g(h),3),n(g(h),8)/n(g(h),7),t{g(h),1}); % CLZ 2011-05-17
          end
        end
