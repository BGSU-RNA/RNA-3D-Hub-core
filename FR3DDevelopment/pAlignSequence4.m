% pAlignSequence4 builds the alignment of a single sequence

function [alignment] = pAlignSequence4(Node,t,maxinsert,n,s,i,j,x)
% x
left        = '';
right      = '';
middle   = '';
...
%   [n active]
  switch Node(n).type,
    ...
    case 'Junction'
     left = [left ...
       Letters(pAlignSequence4(Node,t,maxinsert,Node(n).nextnode(1),s,i,a-1,x),active)];
     right = [...
       Letters(pAlignSequence4(Node,t,maxinsert,Node(n).nextnode(2),s,a,j,x),active) right];
    case 'Alternative'
        % see below
    case 'Basepair'
%         if n==36
...
%        right= [Char(active,length(Node(n).Right(1,:))+sum(maxinsert(n).right)) right];
     ...
     left = [left ...
       Letters(pAlignSequence4(Node,t,maxinsert,Node(n).nextnode(1),s,i,m,x),active)];
   % end left part
   % begining Middle
...
% end Middle
      ...
      right = [...
             Letters(pAlignSequence4(Node,t,maxinsert,Node(n).nextnode(2),s,c,j,x),active) right];       
     end
    ...
    case 'Motif'
     ...
     if (s==1)                   % deleted
      ...
      for a = length(Node(n).Right(1,:)):-1:2,
        right = [Letters(x(j+1-Node(n).Right(r,a)),active) right];
        c=length(Node(n).Right(1,:))-a+1;
        for b=1:maxinsert(n).right(c),
          if b < Node(n).Right(r,c) - Node(n).Right(r,c+1),
            right = [Letters(x(j+1-Node(n).Right(r,c+1)-b),active) right];
          else
