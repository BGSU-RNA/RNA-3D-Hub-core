% pShowStructure(File,NTNumber) displays the secondary structure of a
% molecule as reflected by the interaction matrix, starting at NTNumber and
% ending with whather base NTNumber interacts with

function [Node] = pShowStructure(File,NTNumber,LastNTNumber,Interact,Node,n)
...
G = (abs(File.Inter) < 15) .* (File.Inter ~= 0);    % consider basepairing only
    ...
    if (a < b),                            % if b comes after a
      ...
      if (sum(sum(File.Inter((b+1):(BB-1),(b+1):(BB-1)) == 1)) > 0) & ...
         (sum(sum(File.Inter((a+1):(b-1),(a+1):(b-1))   == 1)) > 0),
           ...
           Node=pShowStructure(File,a,b,Interact,Node,n);
           Node(n).nextnode(2)= length(Node)+1;
           Node=pShowStructure(File,b+1,BB,Interact,Node,length(Node));
        return
      end
