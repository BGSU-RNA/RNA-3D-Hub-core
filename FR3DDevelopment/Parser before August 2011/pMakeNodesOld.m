  ...
  if length(Interact{a}.Categ) > 0,        % if a interacts with something,
    ...
    if (a < b),                            % if b comes after a
      ...
      if (sum(sum(G(t:u,t:u) == 1)) > 0) & ...
         (sum(sum(G(r+1:s-1,r+1:s-1) == 1)) > 0),
           % there are helices between a and b and between b and BB
           n = n+1;

           depth = 4;

           C =   sum(sum(G(r:r+depth,t:t+depth))) ...
               + sum(sum(G(r:r+depth:u-depth:u))) ...
               + sum(sum(G(s-depth:s,t:t+depth))) ...
               + sum(sum(G(s-depth:s,u-depth:u)));
           ...
           if C == 0,
             ...
             rr = r + depth;
             while (sum(G(rr,[t:t+depth u-depth:u])) == 0) && (rr > r),
               rr = rr - 1;
             end

             ss = s - depth;
             while (sum(G(ss,[t:t+depth u-depth:u])) == 0) && (ss < s),
               ss = ss + 1;
             end

             tt = t + depth;
             while (sum(G([r:r+depth s-depth:s],tt)) == 0) && (tt > t),
               tt = tt - 1;
             end

             uu = u - depth;
             while (sum(G([r:r+depth s-depth:s],uu)) == 0) && (uu < u),
               uu = uu + 1;
             end

             % account for additional interactions within loops
