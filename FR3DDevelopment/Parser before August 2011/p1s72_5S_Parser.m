...
if exist('File'),
  [File,FIndex] = zAddNTData('1s72',0,File);
else
  [File,FIndex] = zAddNTData('1s72',0);
end

F = File(FIndex);

nMin = zIndexLookup(F,{'1'},{'9'});    % start of 5S chain
nMax = zIndexLookup(F,{'122'},{'9'});  % end of 5S chain

%zSecondaryStructure(F,nMin,nMax);

% add this interaction to help with Cluster node before it
F = pModifyEdge(F, '27','9', '58','9','cWW');

% remove interactions across the junction until JunctionCluster is fixed
