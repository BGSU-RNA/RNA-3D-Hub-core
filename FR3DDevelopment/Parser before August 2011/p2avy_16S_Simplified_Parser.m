...
Mask = zSimpleSecondaryStructure(File);         % remove pseudoknots and tert

F.Edge = F.Edge .* (Mask > 0);

%  29 Junction Cluster   47   49  361  365  393  393
% These are local crossing interactions which are mis-identified as
% requiring a junction cluster.  However, I don't see how to rewrite that
% code right now to correctly see that it only requires a junction, then
% a cluster node immediately after the junction.

F = pModifyEdge(F, '49', '365','');
F = pModifyEdge(F, '49', '362','');

% this is from another one:

F = pModifyEdge(F, '37','397','');

% and another:

F = pModifyEdge(F, '992','1044','');

% 424 Cluster  957:959 1221:1221
% There is no interaction between strands, only within.
% pMakeNodes isn't able to handle this yet.

F = pModifyEdge(F, '957','959','');

% this causes a JunctionCluster in a loop:

F = pModifyEdge(F, '922','1398','');
