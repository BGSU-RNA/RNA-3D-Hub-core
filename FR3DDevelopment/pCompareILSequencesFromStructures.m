% pCompareSequencesFromStructures reads fasta files from loops extracted
...
for f = current:NumGroups,
  ...
  for n = 1:length(FASTA),
    ...
    if R > 1,
      i = strfind(s,'*');
      Seq{f,1+(n-1)*R} = s([2:(i-2) i (i+2):(end-1)]);
      Seq{f,2+(n-1)*R} = s([(i+2):(end-1) i 2:(i-2)]);

[s ' ' Seq{f,1+(n-1)*R} Seq{f,2+(n-1)*R}]
    else
      Seq{f,n} = s;
    end
  end
