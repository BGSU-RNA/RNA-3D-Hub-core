function Node = pUpdateModelWithSSF(Node,Search,f,F,Verbose,Prior,loopType,File,UseIndex)
        ...
        case 'Initial' % ==========================================================
            if n == 1,
                ...
                if n < length(Node)-1,              % no insertions after last pair
                    ...
                    inscount = zeros(1,L);
                    for c = 1:L,
                        inscount(c) = abs(Search.Candidates(UseIndex(c),aa) - Search.Candidates(UseIndex(c),a)) - 1;
                    end
                    ...
                    for c = 1:L,
                        ...
                        ff = Search.Candidates(UseIndex(c),N+1);          % file number
                        d = Search.Candidates(UseIndex(c),a);             % index of interacting base
                        if inscount(c) > 0,
                            for i = 1:inscount(c),
                    ...
                    if bb==b
                        ...
                        for c = 1:L,
                            inscount(c) = abs(Search.Candidates(UseIndex(c),b) - Search.Candidates(UseIndex(c),bb)) - 1;
                        end
                    end
                    ...
                    for c = 1:L,
                        ...
                        ff = Search.Candidates(UseIndex(c),N+1);          % file number
                        d = Search.Candidates(UseIndex(c),b);            % index of interacting base
                        if inscount(c) > 0,
                            for i = 1:inscount(c),
                                insb = Search.File(ff).NT(d-i).Code; % A=1, C=2, G=3, U=4
                                if ~isempty(insb)
                ...
                if a<b,
                    for i = a:b,
                        ...
                        if isempty(find(interbases==i)),
                            ...
                            clear basesInds
                            for j = 1:L
                                f = Search.Candidates(UseIndex(j),end);
                                if N == length(Search.File(UseIndex(j)).NT),  % if insertions seem to be missing or there are none
                                    baseCode = Search.File(f).NT(i).Code;
                                else
                                    position = Search.Candidates(UseIndex(j),i);
                        ...
                        if i ~= Node(n).RightIndex,
                            ...
                            for j = 1:L
                                ...
                                if inserts ~= 0,
                                    if N ~= length(Search.File(f).NT),
                                        start = Search.Candidates(UseIndex(j),i);
                                        for k = 1:inserts,
                    ...
                    for j = L,
                        f = Search.Candidates(UseIndex(j),end);
                        if ins(j) >= 1,
                            position = Search.Candidates(UseIndex(j),a)+1;
                            baseCode = Search.File(f).NT(position).Code;
                            ...
                            if ins(j) >= 2,
                                for i = 1:ins(j)-1
                                    position = Search.Candidates(UseIndex(j),a)+i+1;
                                    baseCode = Search.File(f).NT(position).Code;
