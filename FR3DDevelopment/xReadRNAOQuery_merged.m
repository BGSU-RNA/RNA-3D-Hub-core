% xReadRNAOQuery reads textfile Filename and converts the RNAO specification there into a FR3D symbolic search data structure Query
    ...
    if L > -1,
      ...
      if ~isempty(strfind(L(1:2),'nt')),         % this line is about an nt
        ...
        elseif length(a) > 1,                        % maybe about two nts
          ...
          if ~isempty(strfind(L,' not ')),         % ignore "not" statements

          elseif ~isempty(strfind(L,'pairs_with_')),     % particular type
          ...
          elseif ~isempty(strfind(L,'cov_bonded_')),         % covalently bonded
            Query.Diff{i,j} = [Query.Diff{i,j} ' =1'];
