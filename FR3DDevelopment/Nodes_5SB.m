n=1;                                            % current node
  ...
  Node(n).lpar          = .01;                        % left insertion parameter
  Node(n).rpar          = .01; 
  Node(n).Bl            = [1];
  Node(n).Br            = [117];
...
n=n+1;                                            % current node
  ...
  Node(n).P         = [0.05*ones(17,1) 0.95*ones(17,1)];
                                                % state to state transitions
  Node(n).PIns	    = [0.05 0.95];              % when no previous state
  Node(n).Left(1,:) = [1 3];                    % nucleotides to use on left
  Node(n).Left(2,:) = [1 4];     
...
n=n+1;                                           % current node
  ...
  n=a-1;
   ...
   Node(n).Left(2,:)    = [1 2 3 5];                                   % nucleotides to use on left
   Node(n).Left(1,:)    = [1 2 4 5];                                   % nucleotides to use on left
   Node(n).Left(2,:)    = [1 3 4 5];                                   % nucleotides to use on left
   Node(n).Left(1,:)    = [1 3 5 6];                                   % nucleotides to use on left
   Node(n).Left(2,:)    = [1 3 6 7];                                   % nucleotides to use on left
   
   Node(n).LIP          = [.05 .1 .1 .05 .2 .95 ];    % probs for insertion possibs
   Node(n).Middle(1,:)  = [1 2 3 4];                                 % nucleotides to use in middle
   Node(n).MIP          = [1];                                          % probs for insertion possibs
   Node(n).Right(1,:)   = [1 2 3];                                       % nucleotides to use on right
   Node(n).RIP          = [1];                                         % probs for insertion possibs
   Node(n).Letters      ='CGGCAGCGGAA';                                  %letters of bases that are interacting
