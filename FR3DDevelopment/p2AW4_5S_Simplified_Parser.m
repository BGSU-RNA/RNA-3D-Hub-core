...
if exist('File'),
  [File,FIndex] = zAddNTData('2AW4',2,File);
else
  [File,FIndex] = zAddNTData('2AW4',2);
end

F = File(FIndex);

nMin = zIndexLookup(F,'2(A)');    % start of 5S chain
nMax = zIndexLookup(F,'118(A)');  % end of 5S chain
...
fprintf('Corrected secondary structure\n');
zSecondaryStructure(F,nMin,nMax);
