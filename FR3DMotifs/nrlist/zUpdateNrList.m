% zUpdateNrList reads a file holding the current list of PDB files and metadata,
...
function [status, err_msg] = zUpdateNrList(filename,current,ReadCode)
    ...
    %     zWriteHTMLFileList(reportdate)              % write out NR lists as HTML
        ...
        err_msg = sprintf('Error "%s" in zUpdateNrList on line %i\n', err.message, err.stack.line);
        disp(err_msg);
        status = 1;
    end
