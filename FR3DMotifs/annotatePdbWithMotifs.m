...
%     exemplar_ids = exemplar_ids(1:20);
    ...
    for i = 1:N
        ...
        for j = 1:E            
            compared_ids = {exemplar_ids{j} loop_ids{i}};
            
            MM = zeros(2,2);
            MM(1,2) = pairwiseSearch(compared_ids{1}, compared_ids{2});
            MM(2,1) = pairwiseSearch(compared_ids{2}, compared_ids{1});
            
            MM = aAnalyzeExtraNucleotides(MM, compared_ids, 0);
            MM = aSymmetrizeMatrix(MM, compared_ids, 0);
            
            if MM(1,2) > 0 && MM(1,2) <=1
                disc(j) = MM(1,2);
            else
                disc(j) = Inf;
            end            
        ...
        [best_match, index] = min(disc);
        result.loop_ids{i} = loop_ids{i};
        if best_match == Inf
            result.best_matches{i} = -1;
        else
            result.best_matches{i} = exemplar_ids{index};
        end       
