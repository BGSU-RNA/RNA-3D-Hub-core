...
class Loader(core.Loader):
        ...
        for chain in rna_chains:
            ...
            if 'rfam_family' in chain:
                if chain['rfam_family'] in rfam_family_to_standardized_name :
                    chain['std_name'] = rfam_family_to_standardized_name[chain['rfam_family']] # add consensus name
                else:
                    if chain['rfam_family'] is not None:
                        if chain['rfam_family'].startswith("RF"):
                            new_rfam.add(chain['rfam_family'])
                if 'domain' in chain:
                    if chain['domain'].startswith("Archaea") and chain['rfam_family'].startswith("RF00169"): # Rule when domain is archaea and it matches RF00169
            ...
            if 'domain' in chain:
                ...
                if chain['domain'].startswith("Archaea"):
                    if '7S.S' not in pdb_compound_name.upper() and "7S" in pdb_compound_name.upper(): # Rule:  if domain == 'Archaea' and '7S' in PDB_compound:
                        standardized_name = "Signal recognition particle RNA; SRP RNA"
                        chain['std_name'] = standardized_name
                    elif 'std_name' not in chain and 'SRP' in pdb_compound_name.upper():
                        standardized_name = "Signal recognition particle RNA; SRP RNA"
                        chain['std_name'] = standardized_name
                ...
                if 'FRAGMENT' in pdb_compound_name.upper() or 'HELIX' in pdb_compound_name.upper() or any('H' +str(num) in pdb_compound_name.upper() for num in helix_set):
                    ...
                    if matching_num is not None: 
                        if 'std_name' in chain:
                            ...
                            short_form = ''
                            if len(chain['std_name'].split(';')) > 1: # if a short name exists
                                short_form = chain['std_name'].split(';')[1]
                            if 'LSU' in short_form:
                                helix_string = "H"
                            elif 'SSU' in short_form:
                                helix_string = "h"
                            long_form += ' Helix %i' % (matching_num)
                            if len(short_form) > 0:
                                short_form = "; " + short_form + ' %s%i' % (helix_string, matching_num)
                            if 'FRAGMENT' not in chain['std_name'].upper():
                                chain['std_name'] = long_form + short_form
                            else:
                                chain['std_name'] = long_form.replace('fragment ', '') + short_form.replace('fragment ', '')
                    else:
                        if 'std_name' in chain:
                            if 'FRAGMENT' not in chain['std_name'].upper():
                                long_form = chain['std_name'].split(';')[0] + ' fragment'
                                short_form = ''
                                if len(chain['std_name'].split(';')) > 1: # if a short name exists
                                    short_form = "; " + chain['std_name'].split(';')[1] + ' fragment'
                                chain['std_name'] = long_form + short_form
    ...
    def make_rfam_family_to_standardized_name(self, standardized_name_list):
        ...
        for line in cn_list:
            ...
            rfam_family_to_standardized_name[line[0]] = standardized_name
        return rfam_family_to_standardized_name
    ...
    def data(self, pdb_id, **kwargs):
        ...
        if rna_query.count() > 0:
            ...
            standardized_arr = [line for line in standardized_names]
            ...
            rfam_family_to_standardized_name = self.make_rfam_family_to_standardized_name(standardized_arr)
            ...
            for chain in rna_chains: #for chain_dict in other_pdb_chains. Check if its an rna chain, if so make this little dictionary if we have (name, doman rfam family), append it to the data list
                ...
                if 'std_name' in chain:
                    ...
                    standardized_name_dict['pdb_id'] = pdb_id
                    standardized_name_dict['chain'] = chain_id
                    standardized_name_dict['property'] = property_val
                    standardized_name_dict['value'] = value
                ...
                if len(standardized_name_dict.values()) > 0:
                    ...
                    # self.logger.info(standardized_name_dict)
