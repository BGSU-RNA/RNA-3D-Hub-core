...
class Loader(core.SimpleLoader):
    ...
    def _get_loop_annotations(self):
        ...
        for loop_type in loop_types:
            ...
            #if a loop instance has either annotation1 and annotation 2 indicated, then do not check for motif annotations
            either_annotation = list(set(loop_annotation1.keys()) | set(loop_annotation2.keys()))

            for loop in either_annotation:
...
#            self.logger.info("Current motif atlas release %s" % current_ml_release)
            ...
            for motif in all_motifs:
                ...
                print("Original loop ids in group %s" % motif)
                print(loop_ids)                    # are there duplicates?
                loop_ids = list(set(loop_ids))     # remove any duplicates
                print("Unique loop ids in this group")
                print(loop_ids)

# if you want to filter by current release, but I think you don't, because annotations of old groups still matter
#                         filter(mod.MlLoops.ml_release_id == current_ml_release)
                ...
                for loop in loop_ids:
                    ...
                    if loop not in either_annotation:
