...
from pymotifs.motif_atlas.cluster import ClusterMotifs
...
class Combiner(core.Base):
    ...
    def __call__(self, release_id, directory):
        ...
        The variable data is a dictonary with the motif group number as the key and
        a dictionary as the value
        """
...
class Known(core.Base):
    ...
    def handles(self,loop_type):
        """
        Get all handles that have ever been used for this loop type
        """

        with self.session() as session:
            query = session.query(mod.MlMotifsInfo.handle).\
                            filter(mod.MlMotifsInfo.type==loop_type).\
                            distinct()

            used_handles = set(result.handle for result in query)

            self.logger.info('Found %d %s handles that have been used for motifs' % (len(used_handles),loop_type))
...
class Builder(core.Base):
    """
    This will build the data structures for a motif release.
    This will load
    ...
    def motifs(self, loop_type, parent_id, release_id, directory):
        ...
        handles = known.handles(loop_type)
    ...
    def __call__(self, loop_type, parent_id, release_id, loop_position_to_border_unit_id,
                 molecule_type = 'RNA', directory=None):
        """
        ...
        if not loop_position_to_border_unit_id and directory is None:
            raise ValueError("Cannot build motifs without loops")

        if not directory:
            cluster = ClusterMotifs(self.config, self.session)
            directory = cluster(loop_type, loop_position_to_border_unit_id, release_id, molecule_type)
