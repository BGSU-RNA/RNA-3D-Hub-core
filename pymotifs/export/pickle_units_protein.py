...
class Exporter(core.Loader):
    ...
    def filename(self,pdb,**kwargs):
        ...
        filename = pdb + '_protein.pickle.gz'
    ...
    def to_process(self, pdbs, **kwargs):
        ...
        with self.session() as session:
            query = session.query(mod.UnitInfo.pdb_id).\
                            filter(mod.UnitInfo.unit_type_id == 'aa').\
                            distinct()
            pdb_ids = set()
            for row in query:
                pdb_ids.add(row.pdb_id)

        # get a list of all files in directory that end with _protein.pickle.gz
        directory = os.path.join(DATA_FILE_DIRECTORY,'units')
        files = [f for f in os.listdir(directory) if f.endswith('_protein.pickle.gz')]
