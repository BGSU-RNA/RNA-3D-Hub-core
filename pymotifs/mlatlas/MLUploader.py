...
from pymotifs.models import MlReleases, MlLoopOrder, LoopPositions,\
...
class Uploader(object):
    ...
    def __init__(self, session_maker, ensembles=None, mode='', description='',
                 files={}, upload_mode='', motif_type=''):

        self.c = ensembles
        self.motif_type = motif_type.upper()
        self.release = MlReleases(mode=mode, description=description,
    ...
    def data(self):
        if self.upload_mode == 'release_diff':
            ...
            return it.chain([self.release], self.release_diff)
        ...
        return it.chain([self.release], self.motifs, self.loops,
                        self.history, self.intersction,
                        self.release_diff, self.loop_order,
                        self.loop_positions, self.loop_discrepancy)
        ...
        for group_id in self.c.c1.sg:
            if group_id in self.c.new_ids:
                motif = MlMotifs(release_id=self.release.id,
                                 ...
                                 type=self.motif_type)
                if group_id in self.c.parents:
                    parents = ','.join(set(self.c.parents[group_id]))
                else:
                    parents = ''
                if self.upload_mode == 'release_diff':
                    # otherwise the new temporary id is committed
                    self.added_groups.append(group_id)
                else:
                ...
                logger.info('Group %s corresponds to motif %s and is '
                            'assigned new id %s', group_id, old_id, motif.id)
                ...
                parents = ''
                logger.info('Group %s matches exactly motif %s', group_id,
                            motif.id)
            else:
            ...
            if parents != '':
                ...
                self.history.append(MlHistory(motif_id=motif.id,
                                              release_id=self.release.id,
                                              parents=parents))
            self.final_ids[group_id] = motif.id
            for loop_id in self.c.c1.d[group_id]:
                self.loops.append(MlLoops(id=loop_id, motif_id=motif.id,
                                          release_id=self.release.id))

            self.removed_groups = set(self.c.c2.groups) - \
                set(self.old_updated_groups) - set(self.same_groups)

    def __store_correspondences(self):
        with open(self.files['correspondences'], 'w') as f:
                ...
                if motif_id != self.final_ids[loop_id] and \
                   ...
                   self.c.intersection[loop_id].has_key(motif_id):
                    ...
                    self.intersection.append(
                        MlSetDiff(motif_id1=self.final_ids[loop_id],
                                  motif_id2=motif_id,
                                  release_id=self.release.id,
    ...
    def __process_motif_loop_order(self):
        ...
        """
        with open(self.files['MotifLoopOrder'], 'rb') as raw:
            r = csv.reader(raw, delimiter=',', quotechar='"')
            for row in r:
                self.loop_order.append(
                    MlLoopOrder(motif_id=self.final_ids[row[0]],
                                loop_id=row[1],
                                release_id=self.release.id,
    ...
    def __process_motif_loop_positions(self):
        ...
        """
        with open(self.files['MotifPositions'], 'rb') as raw:
            r = csv.reader(raw, delimiter=',', quotechar='"')
            for row in r:
                self.loop_positions.append(
                    LoopPositions(motif_id=self.final_ids[row[0]],
                                  loop_id=row[1],
                                  release_id=self.release.id,
                                  nt_id=row[2],
                                  position=row[3]))
    ...
    def __process_bp_signatures(self):
        ...
        """
        with open(self.files['BpSignatures'], 'rb') as raw:
            r = csv.reader(raw, delimiter=',', quotechar='"')
            for row in r:
                motif_id = self.final_ids[row[0]]
                session.merge(MlMotifAnnotations(motif_id=motif_id,
                                                 bp_signature=row[1]))
        session.commit()
    ...
    def __process_mutual_discrepancy(self):
        ...
        """
        with open(self.files['MutualDiscrepancy'], 'rb') as raw:
            r = csv.reader(raw, delimiter=',', quotechar='"')
            for row in r:
                self.loop_discrepancy.append(
                    MlMutualDiscrepancy(loop_id1=row[0],
                                        discrepancy=row[1],
                                        loop_id2=row[2],
                                        release_id=self.release.id))
    ...
    def __rename_mat_files(self):
        ...
        for file in self.c.c1.sg:
            ...
            dst = os.path.join(self.files['MatFiles_dest'],
                               self.final_ids[file] + '.mat')
    ...
    def __move_2d_files(self):
        img_release_folder = os.path.join(self.files['2ds_destination'],
                                          self.motif_type + self.release.id)
    ...
    def __process_graphml_file(self):
        ...
        if os.path.exists(graphml):
            ...
            for group in self.c.c1.sg:
                parts = group.split('_')
                contents = contents.replace('Group_' + parts[1],
                                            self.final_ids[group])
    ...
    # Turning this off as we aren't going to use these annotations
