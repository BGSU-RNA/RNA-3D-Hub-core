...
class Loader(core.SimpleLoader):
    ...
    def to_process(self, pdbs, **kwargs):
        ...
        if self.fill_in_missing:
            ...
            # have a center all at once; do it file by file.
        ...
        if len(pdbs_to_check) == 0:
            raise core.Skip("All pdb ids are already represented in unit_centers table")
    ...
    def query(self, session, pdb):
        ...
        if self.fill_in_missing:
            # return an empty query so we process pdb
    ...
    def data(self, pdb, **kwargs):
        ...
        self.logger.info('Processing %s' % pdb)

        center_to_unit_id = {}
        center_to_unit_id['glycosidic'] = set()
        ...
        if self.fill_in_missing:
            ...
            with self.session() as session:
                ...
                for u in query:
                    if not u.name in center_to_unit_id:
                        center_to_unit_id[u.name] = set()
                    center_to_unit_id[u.name].add(u.unit_id)
            ...
            if len(unit_ids - center_to_unit_id['glycosidic']) == 0:
        ...
        for residue in structure.residues():
            ...
            for name in residue.centers.definitions():
                ...
                if name in center_to_unit_id and residue.unit_id() in center_to_unit_id[name]:
                    # already have this center, so don't add it again
                    continue
                ...
                if len(center) == 3:
                    if self.fill_in_missing:
                        self.logger.info("Adding center %s for %s value %s" % (residue.unit_id(), name, center))
                        print("Adding center %s for %s value %s" % (residue.unit_id(), name, center))
                ...
                if name == 'glycosidic':
                    # only add the rotation matrix after adding a glycosidic atom
                    # then it only gets added once, right?
